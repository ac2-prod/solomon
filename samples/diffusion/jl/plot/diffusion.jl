...
function main()
    ...
    for gpu_id in 1:Ngpu
        ...
        for target in [(fig=perf_max, ax=perf_max.ax[begin]), (fig=compare_max, ax=compare_max.ax[gpu_id])]
            if has_openacc
                plot_lp(target.fig, target.ax, N_acc_par, perf_max_acc_par, point_id=2, line_id=2, color_id=2, label="acc (parallel)")
                plot_lp(target.fig, target.ax, N_acc_ker, perf_max_acc_ker, point_id=4, line_id=3, color_id=2, label="acc (kernels)")
            end
            plot_lp(target.fig, target.ax, N_omp_dist, perf_max_omp_dist, point_id=0, line_id=2, color_id=1, label="omp (distribute)")
            plot_lp(target.fig, target.ax, N_omp_loop, perf_max_omp_loop, point_id=1, line_id=3, color_id=1, label="omp (loop)")
        end
        ...
        perf_max.fig.savefig(string("fig/", "diffusion", "_max", "_", name, ".png"), format="png", dpi=100, bbox_inches="tight")
        perf_med.fig.savefig(string("fig/", "diffusion", "_med", "_", name, ".png"), format="png", dpi=100, bbox_inches="tight")
        if output_pdf
            perf_max.fig.savefig(string("fig/", "diffusion", "_max", "_", name, ".pdf"), format="pdf", bbox_inches="tight")
            perf_med.fig.savefig(string("fig/", "diffusion", "_med", "_", name, ".pdf"), format="pdf", bbox_inches="tight")
        end
