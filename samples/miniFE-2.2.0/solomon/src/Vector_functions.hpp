...
void waxpby(typename VectorType::ScalarType alpha, const VectorType& x,
  ...
  if (beta == 0.0) {
    if (alpha == 1.0) {
      OFFLOAD(AS_INDEPENDENT, ACC_CLAUSE_PRESENT(wcoefs, xcoefs))
      for (int i = 0; i < n; ++i) {
        wcoefs[i] = xcoefs[i];
      }
    } else {
    ...
    if (alpha == 1.0) {
      OFFLOAD(AS_INDEPENDENT, ACC_CLAUSE_PRESENT(wcoefs, xcoefs, ycoefs))
      for (int i = 0; i < n; ++i) {
...
template <typename VectorType>
void daxpby(const MINIFE_SCALAR alpha,
            const VectorType& x,
  ...
  if (alpha == 1.0 && beta == 1.0) {
    OFFLOAD(AS_INDEPENDENT, ACC_CLAUSE_PRESENT(xcoefs, ycoefs))
    for (int i = 0; i < n; ++i) {
      ycoefs[i] += xcoefs[i];
    }
  } else if (beta == 1.0) {
...
dot(const Vector& x,
  ...
  OFFLOAD(REDUCTION(+ : result), OMP_TARGET_CLAUSE_MAP_TOFROM(result), ACC_CLAUSE_PRESENT(xcoefs, ycoefs))
  for (int i = 0; i < n; ++i) {
...
dot_r2(const Vector& x) {
  ...
  OFFLOAD(REDUCTION(+ : result), OMP_TARGET_CLAUSE_MAP_TOFROM(result), ACC_CLAUSE_PRESENT(xcoefs))
  for (int i = 0; i < n; ++i) {
